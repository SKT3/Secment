    ...
    function startHandler($xp, $elem, &$attribs)
    {
        ...
        if (!isset($this->i)) {
            ...
            if (!empty($this->cdata)) {
				...
				$parent->children[] = new XML_Tree_Node(null, $this->cdata); // *** ZZ ***
            }
            $obj_id = 'obj' . $this->i++;
            //$this->$obj_id = &new XML_Tree_Node($elem, null, $attribs);
			$this->$obj_id = new XML_Tree_Node($elem, null, $attribs); // *** ZZ ***
        }
        $this->cdata = null;
        return null;
    }
        ...
        if ($this->i > 1) {
            ...
            if (count($node->children) > 0) {
                if (trim($this->cdata)) {
					...
					$node->children[] = new XML_Tree_Node(null, $this->cdata); // *** ZZ ***
                }
            } else {
