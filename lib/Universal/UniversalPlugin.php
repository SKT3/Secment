...
class UniversalPlugin {
	...
	private $version;
	// 
	// Change $protocol to apropriate value if needed
	//
	private $protocol = "sslv3://";
	//
	private $settings;
	private $timeout;
	...
	function get($name) {
		try {
			...
			if(isset($this->responseData[$name])) {
				return $this->responseData[$name]; // ***ZZ*** 2015.06.01
			}
			else {
				return '';
			}
	...
	private function loadAndParseXMLFile($xmlMapStr) {
		...
		//
		$parser = new UniversalPluginXMLFileParser(); // *** ZZ ***
	...
	private function buildRequest() {
		...
		foreach ($this->xmlMap->request->fields as $field) {
			$data = "";		//  ***ZZ*** 
			if (isset($this->dataMap[$field->id])) { // *** ZZ *** 
	...
	private function sendTransaction($data, $useragent=false) {
		...
		//  
		if (($phpVers == '5') || ($phpVers == '7')) {
			$this->protocol = "tls://"; // *** ZZ *** 
		}
		//---------------------------------------------------------------------------------------------------
		...
		if (!$socket) {
			...
			error_log($this->err);
 			error_log("Unable to connect to $this->protocol$host:$port. $this->err");
	...
	private function parseResponse($responseStr) {
		...
		$zzfields = array_keys($td->responses[0]->fields); 	// ***ZZ*** 2015.06.01
		//
		foreach($this->xmlMap->responses[$idx]->fields as $field) {
		 	$name = strtoupper($field->id); 
			if( in_array($name, $zzfields) ) { 				// ***ZZ*** 2015.06.01
