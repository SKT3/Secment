<?php
	...
	final class MySQLiDriver extends Sql {
		...
		public function connect() {
			...
			$resource = new mysqli($dbhost, $dsn['username'], $dsn['password'], $dsn['database']);
		...
		protected function queryexec($query) {
          ...
          //echo "\n\n $query \n\n";
			...
			// else return the result recorset
				...
				return new MySQLiResult($this->resource, $result);
			}
			return true;
		}
		...
		*/
	    ...
	    public function last_inserted_id($resource, $query) {
			$insert_id = $resource->insert_id;
			if ($insert_id < 0) {
				$insert_id = null;
				$result = $resource->query('SELECT LAST_INSERT_ID()');
				if ($result) {
	    ...
	    public function begin() {
	    	if (!$this->resource->autocommit(false)) {
	...
	class MySQLiResult extends SqlResult {
